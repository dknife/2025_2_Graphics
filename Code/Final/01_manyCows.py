...
def init_gl(window):
    ...
    vert_path = os.path.join('./Final/manyCows/', 'vertex.vs')
    frag_path = os.path.join('./Final/manyCows/', 'frag.fs')
...
def display(window):
    ...
    angle += 0.35
    x = angle * np.pi / 180.0
    camera.eye = np.array([50.0 * np.sin(x), 5+np.sin(10.*x), 50.0 * np.cos(x)])
    ...
    for i in range(100):
        for j in range(100):
            ...
            glUniform2fv(offsetLoc, 1, np.array([i*2-50, j*2-50], dtype=float))
